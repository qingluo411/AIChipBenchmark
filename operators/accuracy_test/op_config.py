...
configs = dict(
    ...
    prelu=(nn.PReLU(), (2, 30, 100),dict(), dict(backward=False)),
    adaptive_avg_pool2d=(nn.AdaptiveAvgPool2d((5, 4)), (1, 64, 30, 8)),
    adaptive_max_pool2d=(nn.AdaptiveMaxPool2d((5, 4)), (1, 64, 30, 80)),
    avgpool2d=(nn.AvgPool2d((3, 2), stride=(2, 1)), (2, 10, 120, 40), dict(), dict(backward=False)),
    ...
    ctc_loss=(nn.CTCLoss(), gen_ctcloss_input(), dict(), dict(backward=False)),  # not for half
    nll_loss=(nn.NLLLoss(), [(1, 16, 100, 10), torch.ones(1, 100, 10).long()]),
    abs=(torch.abs, (2, 3, 50)),
    acos=(torch.acos, (32, 32), dict(), dict(backward=False)),
    asin=(torch.asin, (32, 32), dict(), dict(backward=False)),
    atan=(torch.atan, (32, 32), dict(), dict(backward=False)),
    ceil=(torch.ceil, (3, 32, 10)),
    cos=(torch.cos, (2, 3, 50)),
    cosh=(torch.cosh, (2, 3, 50), dict(), dict(backward=False)),
    erf=(torch.erf, (2, 3, 100), dict(), dict(backward=False)),
    erfc=(torch.erfc, (2, 3, 100), dict(), dict(backward=False)),
    erfinv=(torch.erfinv, (2, 3, 100)),
    exp=(torch.exp, (2, 50)),
    exp2=(torch.exp2, (2, 50, 10), dict(), dict(backward=False)),
    expm1=(torch.expm1, (2, 50), dict(), dict(backward=False)),
    floor=(torch.floor, (2, 3, 50)),
    fmod=(torch.fmod, (2, 3, 50), (2,), dict(backward=False)),
    ...
    logical_xor=(torch.logical_xor, [gen_rand((2, 100), False), gen_rand((2, 100), False)], dict(), dict(backward=False)),
    ...
    repeat=(repeat_wrap, (100, 50)),
    stack=(stack_wrap, [(2, 3, 4), (2, 3, 4)]),
    cat=(cat_wrap, [(2, 3, 4), (2, 3, 4)]),
    fold=(nn.Fold(output_size=(21, 21), kernel_size=(2, 2)), (1, 400, 400), dict(), dict(backward=False)),
    unfold=(nn.Unfold(kernel_size=(2, 3)), (2, 5, 30, 100), dict(), dict(backward=False)),
    ...
    dot=(torch.dot, [(100), (100)], dict(), dict(backward=False)),
    mv=(torch.mv, [(20, 40), (40)]),
    linear=(nn.Linear(20, 30), (128, 20)),
    bilinear=(nn.Bilinear(20, 30, 40), [(128, 20), (128, 30)],dict(), dict(backward=False)),
    mm=(torch.mm, [(20, 30), (30, 50)]),
    ger=(torch.ger, [(10,), (5)]),
    bmm=(torch.bmm, [(2, 3, 400), (2, 400, 60)], dict(), dict(backward=False)),
    ...
    corss=(torch.cross, [(10, 3), (10, 3)], dict(dim=1), dict(backward=False)),
    diagonal=(torch.diagonal, (10, 10), dict(offset=1)),
    trace=(torch.trace, (10, 10), dict(), dict(backward=False)),
    ...
    interpolate=(F.interpolate, (2, 30, 40), dict(scale_factor=2), dict(backward=False)),
    ...
    clamp=(torch.clamp, (2, 3, 100, 100), dict(min=-0.5, max=0.5), dict(backward=False)),
    sign=(torch.sign, (2, 40)),
    all=(torch.all, torch.rand(4, 20).bool(), dict(dim=0)),
    any=(torch.any, torch.rand(4, 20).bool()),
    max=(torch.max, (2, 3, 100, 100),dict(), dict(backward=False)),
    min=(torch.min, (2, 3, 100, 100),dict(), dict(backward=False)),
    mean=(torch.mean, (2, 3, 100, 100)),
    sum=(torch.sum, (2, 3, 100, 100)),
    prod=(torch.prod, (2, 3, 40, 100),dict(), dict(backward=False)),
    median=(torch.median, (2, 3, 100, 100),dict(), dict(backward=False)),
    ...
    normal=(gen_normal, [torch.arange(1., 11.), torch.arange(1, 0, -0.1)]),
    range=(gen_range, [], dict(start=1, end=4, step=0.5)),
    eig=(torch.linalg.eigvals, (3, 2, 2)), 
)
